...
function event_queue:add(e)
    ...
    local t_event_type = "host"
    local t_host_name = broker_cache:get_hostname(e.host_id)
    if t_host_name then
        event_data.host_name =  t_host_name
    else
    ...
    if e.service_id then
        t_event_type = "service"
        local t_service_description = broker_cache:get_service_description(e.host_id, e.service_id)
        if t_service_description then
            event_data.service_description   = broker_cache:get_service_description(e.host_id, e.service_id)
        else
    ...
    event_data.event_type = t_event_type
    splunk_event_data = {
...
-- Fonction init()
function init(conf)
    broker_log:set_parameters(1, "/var/log/centreon-broker/stream-connector-bis.log")
