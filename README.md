...
## stream connector for HP BSM : *bsm/bsm_connector.lua*
...
* loglevel as **number** : the log level (0, 1, 2, 3) where 3 is the maximum level
* max_size as **number** : how many events to store before sending them to the server
* max_age as **number** : flush the events when the specified time (in second) is reach (even if max_size is not reach) max_age as **number** : flush the events when the specified time (in second) is reach (even if max_size is not reach)
