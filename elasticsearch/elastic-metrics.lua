...
--------------------------------------------------------------------------------
function init(conf)
  if conf['log-file'] then
    elastic.log_file = conf['log-file']
  else
    elastic.log_file = '/tmp/elastic-centreon.log'
    broker_log:info(2, "no 'log-file' value given, '/tmp/elastic-centreon.log' set by default")
  end
  broker_log:set_parameters(3, elastic.log_file)
  ...
  if conf['elastic-port'] and conf['elastic-port'] ~= "" then
    ...
    elastic.port = 9200
    broker_log:info(2, "no 'elastic-port' value given, 9200 set by default")
  end
  ...
  if conf['max-row'] then
    ...
    elastic.max_row = 10
    broker_log:info(2, "no 'max-row' value given, 10 set by default")
  end
...
local function flush()
  broker_log:info(2, "flush called with " .. #elastic.rows .. " data")
  local retval = true
  if #elastic.rows > 0 then
    ...
    header = header .. 'Content-Length: '
      .. data:len() .. "\r\n\r\n" .. data
    broker_log:info(3, 'Data sent: ' .. header)
    elastic.socket:write(header)
    local answer = elastic.socket:read()
    local ret
    if answer:match("HTTP/1.1 200 OK") then
      broker_log:info(2, "flush: " .. #elastic.rows .. " data successfully sent")
      ret = true
    else
