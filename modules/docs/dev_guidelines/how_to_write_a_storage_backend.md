...
- [How to write a storage backend](#how-to-write-a-storage-backend)
  ...
  - [Development guidelines](#developement-guidelines)
...
- a stream connector communicates with the sc_storage wrapper. It is up to the sc_storage wrapper to use the appropriate backend to deliver a storage mechanism.
...
- Your backend Lua module must be created in the centreon-stream-connectors-lib/storage_backends directory.
...
When implementing them in your cache backend, you must follow the rules listed below:
...
Stream connectors are mostly working around the following data types:
...
You absolutely must keep this consistent. If you store a `true` boolean for a property, you shouldn't get a `"true"` string when retrieving the value.

For some storage mechanisms it is kind of easy because they work well with Lua data types.
...
- every item of data is stored as a string but the original data type is stored in the data type column
...
For example, the sqlite backend needs a parameter to know the name of the database file name. This parameter is named as follow:
...
This documentation is a good example of how your storage module must be documented.
