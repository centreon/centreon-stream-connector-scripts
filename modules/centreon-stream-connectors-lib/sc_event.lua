...
function ScEvent:is_valid_hostgroup()
  ...
  if not self.event.hostgroups then
    self.sc_logger:debug("[sc_event:is_valid_hostgroup]: dropping event because host with id: " .. tostring(self.event.host_id) 
...
function ScEvent:is_ba_valid()
  ...
  if (not self.event.cache.name and self.params.skip_nil_id)
    or (not self.event.cache.name and self.params.skip_anon_events == 1) then
    return false
  end
