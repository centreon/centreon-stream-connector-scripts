...
function ScLogger:set_params(params)
  self.params = params
  self:notice(self.params.enable_broker_cache_counter_check)
end
...
function ScLogger:log_trace(step, host_id, flush)
  ...
  -- self:notice(self.sc_common:dumper(self.trace_action))
  ...
  if flush and type(self.trace_action) == "table" then
    ...
    local h_name
    -- self:notice(self.sc_common:dumper(self.trace_action))
    for host_id, trace_info in pairs(self.trace_action) do
