...
function sc_cache_engine.new(params, sc_common, sc_logger)
  ...
  if pcall(require, "luasql.mysql") then
    ...
    if not centreon_db then
      self.sc_logger:error("[sc_cache_engine:new]: couldn't connect to " .. tostring(params.centreon_db_name) .. ". Error is: " .. tostring(error)
    ...
    self.sc_logger:error("[sc_cache_engine:new]: couldn't load luasql.mysql module and you asked for it by using the enable_broker_cache_counter_check parameter."
...
function ScCacheEngine:get_host_all_infos(host_id)
  -- return because host_id isn't valid
  if host_id == nil or host_id == "" then
    self.sc_logger:warning("[sc_cache_engine:get_host_all_infos]: host id is nil")
    return false
  end
  ...
  if not host_info then
    self.sc_logger:error("[sc_cache_engine:get_host_all_infos]: couldn't find host: " .. tostring(host_id) 
...
function ScCacheEngine:get_service_all_infos(host_id, service_id)
  -- return because host_id or service_id isn't valid
  if host_id == nil or host_id == "" or service_id == nil or service_id == "" then
    self.sc_logger:warning("[sc_cache_engine:get_service_all_infos]: host id or service id is nil")
    return false
  end
  ...
  if not service_info then
    self.sc_logger:error("[sc_cache_engine:get_host_all_infos]: couldn't find service: " .. tostring(service_id) 
      .. " in your database. Maybe it has been disabled or removed. You should export your configuration.")
    return false
  end
...
function ScCacheEngine:get_host_infos(host_id, info)
  -- return because host_id isn't valid
  if host_id == nil or host_id == "" then
    self.sc_logger:warning("[sc_cache_engine:get_host_infos]: host id is nil")
    return false
  end
  ...
  if not host_info then
    self.sc_logger:warning("[sc_cache_engine:get_host_infos]: No host information found for host_id:  " .. tostring(host_id) .. ". Restarting centengine should fix this.")  
    return host
  end
...
function ScCacheEngine:get_service_infos(host_id, service_id, info)
  -- return because host_id or service_id isn't valid
  if host_id == nil or host_id == "" or service_id == nil or service_id == "" then
    self.sc_logger:warning("[sc_cache_engine:get_service_infos]: host id or service id is invalid")
    return false
  end
  ...
  if not service_info then
    self.sc_logger:warning("[sc_cache_engine:get_service_infos]: No service information found for host_id:  " .. tostring(host_id) .. " and service_id: " .. tostring(service_id) 
...
function ScCacheEngine:get_hostgroups(host_id)
  -- return false if host id is invalid
  if host_id == nil or host_id == "" then 
    self.sc_logger:warning("[sc_cache_engine:get_hostgroup]: host id is nil or empty")
    return false
  end
...
function ScCacheEngine:get_servicegroups(host_id, service_id)
  -- return false if service id is invalid
  if host_id == nil or host_id == "" or service_id == nil or service_id == "" then 
    self.sc_logger:warning("[sc_cache_engine:get_servicegroups]: service id is nil or empty")
    return false
  end
...
function ScCacheEngine:get_severity(host_id, service_id)
  -- return false if host id is invalid
  if host_id == nil or host_id == "" then 
    self.sc_logger:warning("[sc_cache_engine:get_severity]: host id is nil or empty")
    return false
  end
  ...
  if service_id == nil then
    ...
    if not severity then
      self.sc_logger:warning("[sc_cache_engine:get_severity]: no severity found in broker cache for host: " .. tostring(host_id))
      return false
    end
  ...
  -- return false if no severity were found
  if not severity then
    self.sc_logger:warning("[sc_cache_engine:get_severity]: no severity found in broker cache for host id: " .. tostring(host_id) .. " and service id: " .. tostring(service_id))
    return false
  end
...
function ScCacheEngine:get_instance(instance_id)
  -- return false if instance_id is invalid
  if instance_id == nil or instance_id == "" then
    self.sc_logger:warning("[sc_cache_engine:get_instance]: instance id is nil or empty")
    return false
  end
  ...
  if not name then
    self.sc_logger:warning("[sc_cache_engine:get_instance]: couldn't get instance name from broker cache for instance id: " .. tostring(instance_id))
    return false
  end
...
function ScCacheEngine:get_ba_infos(ba_id)
  -- return false if ba_id is invalid
  if ba_id == nil or ba_id == "" then 
    self.sc_logger:warning("[sc_cache_engine:get_ba_infos]: ba id is nil or empty")
    return false
  end
  ...
  if ba_info == nil then
    self.sc_logger:warning("[sc_cache_engine:get_ba_infos]: couldn't get ba informations in cache for ba_id: " .. tostring(ba_id))
    return false
  end
...
function ScCacheEngine:get_bvs_infos(ba_id)
  -- return false if ba_id is invalid
  if ba_id == nil or ba_id == "" then 
    self.sc_logger:warning("[sc_cache_engine:get_bvs]: ba id is nil or empty")
    return false
  end
  ...
  if bvs_id == nil or bvs_id == "" then
    self.sc_logger:warning("[sc_cache_engine:get_bvs]: couldn't get bvs for ba id: " .. tostring(ba_id))
    return false
  end
  ...
  for _, id in ipairs(bvs_id) do
    ...
    if bv_infos then
      ...
      self.sc_logger:warning("[sc_cache_engine:get_bvs]: couldn't get bv information for bv id: " .. tostring(bv_id))
    end
  end
...
function ScCacheEngine:get_centreon_db_info(query)
  local result, error = self.centreon_db:execute(query)

  if not result then
    self.sc_logger:error("[sc_cache_engine:get_centreon_db_info]: query: " .. tostring(query) .. " failed\n error: " .. tostring(error))
    return nil
  end
