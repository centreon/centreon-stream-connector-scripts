...
local config = require 'centreon-stream-connectors-lib.rdkafka.config'.new()
...
local topic_config = require 'centreon-stream-connectors-lib.rdkafka.topic_config'.new()
topic_config["auto.commit.enable"] = "true"

local topic = require 'centreon-stream-connectors-lib.rdkafka.topic'.new(producer, TOPIC_NAME, topic_config)
