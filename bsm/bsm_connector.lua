...
local my_data = {
  ...
  logfile = "/var/log/centreon-broker/bsm_connector.log",
...
local function flush()
  ...
  local buf = '<event_list xmlns="http://www.hp.com/2009/software/opr/data_model">' .. "\n" .. table.concat(my_data.data, "\n") .. '</event_list>'
  local respbody = {}
  local  body, code, headers, status = http.request {
    ...
    headers =
             {
               ["Content-Type"] = "Content-Type:text/xml; type=collection",
...
function write(d)
  ...
  if d.category == 1 and d.element == 24 then
    ...
    if d.host_id and d.service_id then
      ...
      -- handling the case when notes_url is empty
      local notes_url = ''
      if d.notes_url ~= nil then
        ...
        if d.last_hard_state_change then
          if math.abs(d.last_check - d.last_hard_state_change) < 10 then --we keep only events with a state that changed from the previous check
            ...
            local reqbody = "<event_data>" .. 
                "<svc_desc>" .. service_desc .. "</svc_desc>" .. 
                "<output>" .. d.output .. "</output>" .. 
                "<state>" .. d.state .. "</state>" .. 
                "<last_update>" .. d.last_update .. "</last_update>" .. 
                "<hostname>" .. hostname .. "</hostname>" .. 
                "<notes_url>" .. notes_url .. "</notes_url>" .. 
                "<source_ci>" .. my_data.source_ci .. "</source_ci>" .. 
                "<source_host_id>" .. d.host_id .. "</source_host_id>" .. 
                "<source_svc_id>" .. d.service_id .. "</source_svc_id>" .. 
                "<scheduled_downtime_depth>" .. d.scheduled_downtime_depth .. "</scheduled_downtime_depth>" .. 
                "</event_data>"
            table.insert(my_data.data, reqbody)
    	    broker_log:info(3, "write XML: " .. reqbody)
            end
          end
        end
      end
